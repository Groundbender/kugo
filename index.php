<!DOCTYPE html>
<html lang="ru">
<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="stylesheet" href="css/normalize.css">
  <link rel="stylesheet" href="css/style.css">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Jost:wght@400;500;600;700&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="./css/swiper-bundle.min.css">
  <link rel="icon" href="img/favicon.ico" type="image/x-icon">
  <title>Kugo</title>
</head>
<body>
  <div class="mobile-menu  ">
    <ul class="mobile-menu-nav">
       
      <li class="mobile-menu-nav-item">
        <a href="#" class="mobile-menu-link">Сервис</a>
      </li>
      <li class="mobile-menu-nav-item">
        <a href="#" class="mobile-menu-link">Сотрудничество</a>
     
      </li>
      <li class="mobile-menu-nav-item">
        <a href="#" class="mobile-menu-link">Заказать звонок</a>
      </li>
      <li class="mobile-menu-nav-item">
        <a href="#" class="mobile-menu-link">Каталог товаров</a>
        <ul class="mobile-submenu">
          <li class="mobile-submenu-item">
            <a class="mobile-submenu-link" href="#">Электросамокаты</a>
          </li>
          <li class="mobile-submenu-item">
            <a class="mobile-submenu-link" href="#">Электроскутеры</a>
          </li>
          <li class="mobile-submenu-item">
            <a class="mobile-submenu-link" href="#">Электровелосипеды</a>
          </li>
        </ul>
      </li>
      <li class="mobile-menu-nav-item">
        <a href="#" class="mobile-menu-link">Покупателям</a>
        <ul class="mobile-submenu">
          <li class="mobile-submenu-item">
            <a class="mobile-submenu-link" href="#">Сервисный центр</a>
          </li>
          <li class="mobile-submenu-item">
            <a class="mobile-submenu-link" href="#">Рассрочка</a>
          </li>
          <li class="mobile-submenu-item">
            <a class="mobile-submenu-link" href="#">Тест-драйв</a>
          </li>
        </ul>
      </li>
      <hr class="mobile-seporater">
        <h4 class="mobile-contacts-title">
          Контакты
        </h4>
      <div class="contacts-wrapper-mobile">
        <div class="call-center">
          <p class="mobile-contacts-text">Call-центр</p>
          <a href="tel:+78005055461" class="mobile-phone">+7 (800) 505-54-61</a>
          <span class="contacts-description">Пн-Вс 10:00 - 20:00</span>
        </div>
        <div class="service-center">
          <p class="mobile-contacts-text">Сервис-центр</p>
          <a href="tel:+78005055461" class="mobile-phone">+7 (499) 350-76-92</a>
          <span class="contacts-description">Пн-Вс 10:00 - 20:00</span>
        </div>
      </div>
      <div class="social-wrapper-mobile">
        <div class="social-block-mobile">
          <svg class="social-icon"  width="20" height="20">
            <use href="img/sprite.svg#vk"></use>
          </svg>
          <div class="social-content">
            <p class="social-media-text">Вконтакте</p>
            <span class="social-media-description">3 300</span>
          </div>
        </div>
        <div class="social-block-mobile">
          <svg class="social-icon"  width="20" height="20">
            <use href="img/sprite.svg#yb"></use>
          </svg>
          <div class="social-content">
            <p class="social-media-text">YouTube</p>
            <span class="social-media-description">3 603</span>
          </div>
        </div>
        <div class="social-block-mobile">
          <svg class="social-icon"  width="20" height="20">
            <use href="img/sprite.svg#tg-blue"></use>
          </svg>
          <div class="social-content">
            <p class="social-media-text">Telegram</p>
            <span class="social-media-description">432</span>
          </div>
        </div>
      </div>
  </div>
  
  <header>
    <nav class="mobile-navbar ">
      <a href="#" class="mobile-menu-toggle">
        <div class="mobile-menu-line-container">
        <div class="mobile-menu-line"></div>
        <div class="mobile-menu-line"></div>
        <div class="mobile-menu-line"></div>
        </div>
      </a>
      <a href="./" class="navbar-logo">KUGOO</a>
      <a href="tel:+78005055461" class="navbar-phone">+7 (800) 505-54-61</a>
     

    </nav>
    <nav class="navbar">
      <div class="container">
              <div class="navbar-wrapper">
              <ul class="header-nav-top">
                <li class="navbar-item">
                 <a href="#" class="navbar-link">Сервис</a>
                 </li>
                 <li class="navbar-item">
                 <a href="#" class="navbar-link">Сотрудничество</a>
                 </li>
                 <li class="navbar-item">
                 <a href="#" class="navbar-link">Заказать звонок</a>
                 </li>
                 <li class="navbar-item navbar-logo-links">
                   <a href="#" class="navbar-logo-link">
                     <svg class="social-icon"  width="12" height="12">
                       <use href="img/sprite.svg#viber"></use>
                     </svg>
                   </a>
                   <a href="#" class="navbar-logo-link">
                     <svg class="social-icon"  width="12" height="12">
                       <use href="img/sprite.svg#whats"></use>
                     </svg>
                   </a>
                   <a href="#" class="navbar-logo-link">
                     <svg class="social-icon"  width="12" height="12">
                       <use href="img/sprite.svg#tg"></use>
                     </svg>
                   </a>
                 </li>
               </ul>
               <a href="tel:+78005055461" class="navbar-phone">+7 (800) 505-54-61</a>
          </div>
      </div>
    </nav>

      <div class="header-nav">
        <div class="container">
          <div class="header-nav-wrapper">
            <a href="#" class="header-logo">KUGOO</a>
        <a href="#catalog" class="button header-button">
         <div class="navbar-button-lines-wrapper">
          <span class="button-line"></span>
          <span class="button-line"></span>
          <span class="button-line"></span>
         </div>
          <p class="button-text">Каталог</p>
        </a>
        <form class="search-form" action="#" >
          <input name="search" placeholder="Искать самокат KUGO" type="search" class="input-search">
          <button class="button-search"type="submit">
            <svg class="social-icon"  width="16" height="16">
              <use href="img/sprite.svg#loop"></use>
            </svg>
          </button>
        </form>
        <div class="header-basket-wrapper">
         <a href="#" class="navbar-icon "  >
         <svg class="header-nav-icon" >
            <use href="img/sprite.svg#scales"></use>
          </svg>
         
         </a>
         <a href="#" class="navbar-icon">
          <svg class="header-nav-icon" >
            <use href="img/sprite.svg#heart"></use>
          </svg>
         </a>
         <a href="#" class="basket-link">
          <svg class="header-nav-icon"  width="20" height="20">
            <use href="img/sprite.svg#basket"></use>
          </svg>
          <span class="basket-link-text">Корзина</span>
         </a> 
         
        </div>
      </div>
      <!-- navbar-bottom-wrapper  -->
      </div>
      <!-- container  -->
      </div>
      <!-- navbar-bottom  -->

      
  </header>
  <main class="main">
  <div class="main-content">
    <div class="location-content-wrapper">
      <svg class="social-icon"  width="15" height="15">
        <use href="img/sprite.svg#location"></use>
      </svg>
      <address class="address">Восточно-Кругликовская улица, 86 </address>
      
      <span class="work-time" >    Вт - Сб 10:00 - 20:00</span>
    </div>
    <h1 class="main-title">
      Запишитесь на бесплатный <br class="title-space"> тест-драйв электросамокатов
    </h1>
    <p class="main-text">
      в Москве без ограничения по времени
    </p>
    <div class="main-bottom-wrapper">
    <ul class="main-features">
      <li class="main-features-item">
         <div class="features-logo ">
          <svg class="features-icon"  width="16" height="16">
            <use href="img/sprite.svg#scooter"></use>
          </svg>
        </div> 
        <p class="features-text">Поймете, какая <br> модель вам подходит</p>
      </li>
      <li class="main-features-item">
       <div class="features-logo ">
        <svg class="features-icon"  width="16" height="16">
          <use href="img/sprite.svg#light"></use>
        </svg>
      </div> 
     
      <p class="features-text">Проверите лучшие самокаты в деле</p>

      </li>
    </ul>
    <button class="button main-button" data-toggle="modal" data-target="#feedback-modal">
      <span class="main-button-text">Записаться</span>
    </button>
  </div>
  </div> 
  </main>
<section class="section test-drive">
  <div class="container">
<div class="test-drive-wrapper">
  <div class="test-drive-content">
    <h2 class="section-title">Определите максимально подходящую вам модель
      <br> не теоретически, а на практике</h2>
      <p class="list-description">Тест-драйв поможет:</p>
      <ul class="section-list">
        <li class="section-item">
           <svg class="section-icon"  >
          <use href="img/sprite.svg#ok"></use>
        </svg>
        <p class="section-text"> <span class="bold-text">Понять</span>  подходит ли вам конкретная модель;</p>
        </li>
        <li class="section-item">
           <svg class="section-icon"  >
          <use href="img/sprite.svg#ok"></use>
        </svg>
        <p class="section-text"> <span class="bold-text">Испытать</span> самокат в «реальной жизни»;</p>
        </li>
        <li class="section-item">
           <svg class="section-icon"  >
          <use href="img/sprite.svg#ok"></use>
        </svg>
        <p class="section-text"> <span class="bold-text">Оценить</span> удобство хранения и эксплуатации;</p>
        </li>
        <li class="section-item">
           <svg class="section-icon"  >
          <use href="img/sprite.svg#ok"></use>
        </svg>
        <p class="section-text"> <span class="bold-text">Узнать</span> реальные характеристики и возможности модели.</p>
        </li>
      </ul>
  </div>
  <!-- test-drive-content  -->

  <picture>
    <source type="image/webp" srcset="img/test-drive.webp">
    <source  type="image/png" srcset="img/test-drive.jpg">
    <img src="img/test-drive.jpg" alt="test-drive-image" class="test-drive-image">

    </picture>
</div>
<!-- test-drive-wrapper  -->
</div>
<!-- container  -->
</section>


<section class="section instruction">
 
  <div class="container">
<div class="instruction-wrapper">
<picture>
    <source type="image/webp" srcset="img/tutorial.webp">
    <source  type="image/png" srcset="img/tutorial.jpg">
    <img src="img/tutorial.jpg" alt="instruction-image" class="instruction-image">

    </picture>
 
  
  <div class="instruction-content">
    <h2 class="section-title">Научим правильной <br> и безопасной езде в городе вас или вашего ребенка</h2>
      <p class="list-description">На обучении специалист расскажет:</p>
      <ul class="section-list">
        <li class="section-item">
           <svg class="section-icon"  >
          <use href="img/sprite.svg#ok"></use>
        </svg>
        <p class="section-text"> Как подготовить самокат к поездке;</p>
        </li>
        <li class="section-item">
           <svg class="section-icon"  >
          <use href="img/sprite.svg#ok"></use>
        </svg>
        <p class="section-text"> Как «завести» самокат;</p>
        </li>
        <li class="section-item">
           <svg class="section-icon"  >
          <use href="img/sprite.svg#ok"></use>
        </svg>
        <p class="section-text"> Как вести себя во время поездки и обезопасить себя и окружающих;</p>
        </li>
        <li class="section-item">
           <svg class="section-icon"  >
          <use href="img/sprite.svg#ok"></use>
        </svg>
        <p class="section-text"> Как складывать и раскладывать электросамокат;</p>
        </li>
        <li class="section-item">
           <svg class="section-icon"  >
          <use href="img/sprite.svg#ok"></use>
        </svg>
        <p class="section-text"> Как ухаживать за своим девайсом.</p>
        </li>
      </ul>
  </div>
  <!-- test-drive-content  -->

  
</div>
<!-- test-drive-wrapper  -->
</div>
<!-- container  -->
</section>

    
   

 
  
  
  

<section class="section catalog " id="catalog">
  

  <div class="container">
    <h2 class="section-title catalog-title">Сейчас для тест-драйва и  обучения <br> доступны следующие модели</h2>
    
      
    <div class="catalog-desktop">
    <div class="  cards-wrapper">
      <div class="card  ">
        <div class="card-mark"><span class="card-mark-text">ХИТ</span></div>
        
        <div class="card-top">
          <a href="#" class="card-icon card-icon-light">
            <svg class="card-bottom-icon"  width="20" height="20">
              <use href="img/sprite.svg#scales-card"></use>
            </svg>  
          </a>
         
           
          <picture>
    <source type="image/webp" srcset="img/scooter-card.webp">
    <source  type="image/png" srcset="img/scooter-card.png">
    <img src="img/scooter-card.png" alt="scooter photo" class="card-image">

    </picture>
        </div>

          <div class="card-bottom">
            <h4 class=" card-title">Kugoo Kirin M4</h4>
            <ul class="card-description-wrapper">
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#accum"></use>
                </svg>
                <p class="card-description-text">2000 mAh</p>
              </li>
              
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#speedometer"></use>
                </svg>
                <p class="card-description-text">60 км/ч</p>

              </li>
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#card-light"></use>
                </svg>
                <p class="card-description-text">1,2 л.с.</p>

              </li>
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#timer"></use>
                </svg>
                <p class="card-description-text">5 часов</p>

              </li>
            </ul>
            <div class="price-block-wrapper">
              <div class="price-wrapper">
                <span class="last-price">39 900 ₽</span>
                <span class="price-now">29 900 ₽</span>
              </div>
              <div class="price-icons-wrapper">
                <a href="#" class="card-icon">
                  <svg class="section-icon"  width="20" height="20">
                    <use href="img/sprite.svg#card-basket"></use>
                  </svg>
                </a>
                <a href="#" class="card-icon">
                  <svg class="section-icon"  width="20" height="20">
                    <use href="img/sprite.svg#card-heart"></use>
                  </svg>
                </a>
              </div>
            </div>
           <div class="button-wrapper">
            <button class="button card-button" data-toggle="modal" data-target="#feedback-modal"><span class="card-button-text">Записаться на тест-драйв</span></button>
           </div>
          </div>
      </div>
      <div class="card card-even  ">
        <div class="card-mark"><span class="card-mark-text">Новинка</span></div>
        
        <div class="card-top ">
          <a href="#" class="card-icon card-icon-light">
            <svg class="section-icon"  width="20" height="20">
              <use href="img/sprite.svg#scales-card"></use>
            </svg>  
          </a>
         
          
          <picture>
    <source type="image/webp" srcset="img/scooter-card.webp">
    <source  type="image/png" srcset="img/scooter-card.png">
    <img src="img/scooter-card.png" alt="scooter photo" class="card-image">

    </picture>
        </div>

          <div class="card-bottom">
            <h4 class=" card-title">Kugoo Kirin M4</h4>
            <ul class="card-description-wrapper">
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#accum"></use>
                </svg>
                <p class="card-description-text">2000 mAh</p>
              </li>
              
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#speedometer"></use>
                </svg>
                <p class="card-description-text">60 км/ч</p>

              </li>
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#card-light"></use>
                </svg>
                <p class="card-description-text">1,2 л.с.</p>

              </li>
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#timer"></use>
                </svg>
                <p class="card-description-text">5 часов</p>

              </li>
            </ul>
            <div class="price-block-wrapper">
              <div class="price-wrapper">
                <span class="last-price">39 900 ₽</span>
                <span class="price-now">29 900 ₽</span>
              </div>
              <div class="price-icons-wrapper">
                <a href="#" class="card-icon">
                  <svg class="section-icon"  width="20" height="20">
                    <use href="img/sprite.svg#card-basket"></use>
                  </svg>
                </a>
                <a href="#" class="card-icon">
                  <svg class="section-icon"  width="20" height="20">
                    <use href="img/sprite.svg#card-heart"></use>
                  </svg>
                </a>
              </div>
            </div>
           <div class="button-wrapper">
            <button class="button card-button" data-toggle="modal" data-target="#feedback-modal"><span class="card-button-text">Записаться на тест-драйв</span></button>
           </div>
          </div>
      </div>
      <div class="card  ">
        <div class="card-mark"><span class="card-mark-text">ХИТ</span></div>
        
        <div class="card-top">
          <a href="#" class="card-icon card-icon-light">
            <svg class="section-icon"  width="20" height="20">
              <use href="img/sprite.svg#scales-card"></use>
            </svg>  
          </a>
         
          
          <picture>
    <source type="image/webp" srcset="img/scooter-card.webp">
    <source  type="image/png" srcset="img/scooter-card.png">
    <img src="img/scooter-card.png" alt="scooter photo" class="card-image">

    </picture>
        </div>

          <div class="card-bottom">
            <h4 class=" card-title">Kugoo Kirin M4</h4>
            <ul class="card-description-wrapper">
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#accum"></use>
                </svg>
                <p class="card-description-text">2000 mAh</p>
              </li>
              
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#speedometer"></use>
                </svg>
                <p class="card-description-text">60 км/ч</p>

              </li>
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#card-light"></use>
                </svg>
                <p class="card-description-text">1,2 л.с.</p>

              </li>
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#timer"></use>
                </svg>
                <p class="card-description-text">5 часов</p>

              </li>
            </ul>
            <div class="price-block-wrapper">
              <div class="price-wrapper">
                <span class="last-price">39 900 ₽</span>
                <span class="price-now">29 900 ₽</span>
              </div>
              <div class="price-icons-wrapper">
                <a href="#" class="card-icon">
                  <svg class="section-icon"  width="20" height="20">
                    <use href="img/sprite.svg#card-basket"></use>
                  </svg>
                </a>
                <a href="#" class="card-icon">
                  <svg class="section-icon"  width="20" height="20">
                    <use href="img/sprite.svg#card-heart"></use>
                  </svg>
                </a>
              </div>
            </div>
           <div class="button-wrapper">
            <button class="button card-button" data-toggle="modal" data-target="#feedback-modal"><span class="card-button-text">Записаться на тест-драйв</span></button>
           </div>
          </div>
      </div>
      <div class="card card-even  ">
        <div class="card-mark"><span class="card-mark-text">Новинка</span></div>
        
        <div class="card-top">
          <a href="#" class="card-icon card-icon-light">
            <svg class="section-icon"  width="20" height="20">
              <use href="img/sprite.svg#scales-card"></use>
            </svg>  
          </a>
         
          <picture>
    <source type="image/webp" srcset="img/scooter-card.webp">
    <source  type="image/png" srcset="img/scooter-card.png">
    <img src="img/scooter-card.png" alt="scooter photo" class="card-image">

    </picture>
        </div>

          <div class="card-bottom">
            <h4 class=" card-title">Kugoo Kirin M4</h4>
            <ul class="card-description-wrapper">
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#accum"></use>
                </svg>
                <p class="card-description-text">2000 mAh</p>
              </li>
              
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#speedometer"></use>
                </svg>
                <p class="card-description-text">60 км/ч</p>

              </li>
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#card-light"></use>
                </svg>
                <p class="card-description-text">1,2 л.с.</p>

              </li>
              <li class="card-description">
                <svg class="card-bottom-icon"  width="18" height="18">
                  <use href="img/sprite.svg#timer"></use>
                </svg>
                <p class="card-description-text">5 часов</p>

              </li>
            </ul>
            <div class="price-block-wrapper">
              <div class="price-wrapper">
                <span class="last-price">39 900 ₽</span>
                <span class="price-now">29 900 ₽</span>
              </div>
              <div class="price-icons-wrapper">
                <a href="#" class="card-icon">
                  <svg class="section-icon"  width="20" height="20">
                    <use href="img/sprite.svg#card-basket"></use>
                  </svg>
                </a>
                <a href="#" class="card-icon">
                  <svg class="section-icon"  width="20" height="20">
                    <use href="img/sprite.svg#card-heart"></use>
                  </svg>
                </a>
              </div>
            </div>
           <div class="button-wrapper">
            <button class="button card-button" data-toggle="modal" data-target="#feedback-modal"><span class="card-button-text">Записаться на тест-драйв</span></button>
           </div>
          </div>
      </div>
    </div>
    </div>
    <div class="swiper">
      <!-- Additional required wrapper -->
      <div class="swiper-wrapper  cards-wrapper">
        <!-- Slides -->
        <div class="swiper-slide"><div class="card  ">
          <div class="card-mark"><span class="card-mark-text">ХИТ</span></div>
          
          <div class="card-top">
            <a href="#" class="card-icon card-icon-light">
              <svg class="section-icon"  width="20" height="20">
                <use href="img/sprite.svg#scales-card"></use>
              </svg>  
            </a>
           
            <picture>
    <source type="image/webp" srcset="img/scooter-card.webp">
    <source  type="image/png" srcset="img/scooter-card.png">
    <img src="img/scooter-card.png" alt="scooter photo" class="card-image">

    </picture>
          </div>
  
            <div class="card-bottom">
              <h4 class=" card-title">Kugoo Kirin M4</h4>
              <ul class="card-description-wrapper">
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#accum"></use>
                  </svg>
                  <p class="card-description-text">2000 mAh</p>
                </li>
                
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#speedometer"></use>
                  </svg>
                  <p class="card-description-text">60 км/ч</p>
  
                </li>
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#card-light"></use>
                  </svg>
                  <p class="card-description-text">1,2 л.с.</p>
  
                </li>
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#timer"></use>
                  </svg>
                  <p class="card-description-text">5 часов</p>
  
                </li>
              </ul>
              <div class="price-block-wrapper">
                <div class="price-wrapper">
                  <span class="last-price">39 900 ₽</span>
                  <span class="price-now">29 900 ₽</span>
                </div>
                <div class="price-icons-wrapper">
                  <a href="#" class="card-icon">
                    <svg   width="20" height="20">
                      <use href="img/sprite.svg#card-basket"></use>
                    </svg>
                  </a>
                  <a href="#" class="card-icon">
                    <svg   width="20" height="20">
                      <use href="img/sprite.svg#card-heart"></use>
                    </svg>
                  </a>
                </div>
              </div>
             <div class="button-wrapper">
              <button class="button card-button" data-toggle="modal" data-target="#feedback-modal"><span class="card-button-text">Записаться на тест-драйв</span></button>
             </div>
            </div>
        </div></div>
        <div class="swiper-slide"> <div class="card card-even  ">
          <div class="card-mark"><span class="card-mark-text">Новинка</span></div>
          
          <div class="card-top ">
            <a href="#" class="card-icon card-icon-light">
              <svg class="section-icon"  width="20" height="20">
                <use href="img/sprite.svg#scales-card"></use>
              </svg>  
            </a>
           
            <picture>
    <source type="image/webp" srcset="img/scooter-card.webp">
    <source  type="image/png" srcset="img/scooter-card.png">
    <img src="img/scooter-card.png" alt="scooter photo" class="card-image">

    </picture>
          </div>
  
            <div class="card-bottom">
              <h4 class=" card-title">Kugoo Kirin M4</h4>
              <ul class="card-description-wrapper">
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#accum"></use>
                  </svg>
                  <p class="card-description-text">2000 mAh</p>
                </li>
                
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#speedometer"></use>
                  </svg>
                  <p class="card-description-text">60 км/ч</p>
  
                </li>
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#card-light"></use>
                  </svg>
                  <p class="card-description-text">1,2 л.с.</p>
  
                </li>
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#timer"></use>
                  </svg>
                  <p class="card-description-text">5 часов</p>
  
                </li>
              </ul>
              <div class="price-block-wrapper">
                <div class="price-wrapper">
                  <span class="last-price">39 900 ₽</span>
                  <span class="price-now">29 900 ₽</span>
                </div>
                <div class="price-icons-wrapper">
                  <a href="#" class="card-icon">
                    <svg  width="20" height="20">
                      <use href="img/sprite.svg#card-basket"></use>
                    </svg>
                  </a>
                  <a href="#" class="card-icon">
                    <svg  width="20" height="20">
                      <use href="img/sprite.svg#card-heart"></use>
                    </svg>
                  </a>
                </div>
              </div>
             <div class="button-wrapper">
              <button class="button card-button" data-toggle="modal" data-target="#feedback-modal"><span class="card-button-text">Записаться на тест-драйв</span></button>
             </div>
            </div>
        </div></div>
        <div class="swiper-slide"><div class="card  ">
          <div class="card-mark"><span class="card-mark-text">ХИТ</span></div>
          
          <div class="card-top">
            <a href="#" class="card-icon card-icon-light">
              <svg class="section-icon"  width="20" height="20">
                <use href="img/sprite.svg#scales-card"></use>
              </svg>  
            </a>
           
            <picture>
    <source type="image/webp" srcset="img/scooter-card.webp">
    <source  type="image/png" srcset="img/scooter-card.png">
    <img src="img/scooter-card.png" alt="scooter photo" class="card-image">

    </picture>
          </div>
  
            <div class="card-bottom">
              <h4 class=" card-title">Kugoo Kirin M4</h4>
              <ul class="card-description-wrapper">
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#accum"></use>
                  </svg>
                  <p class="card-description-text">2000 mAh</p>
                </li>
                
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#speedometer"></use>
                  </svg>
                  <p class="card-description-text">60 км/ч</p>
  
                </li>
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#card-light"></use>
                  </svg>
                  <p class="card-description-text">1,2 л.с.</p>
  
                </li>
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#timer"></use>
                  </svg>
                  <p class="card-description-text">5 часов</p>
  
                </li>
              </ul>
              <div class="price-block-wrapper">
                <div class="price-wrapper">
                  <span class="last-price">39 900 ₽</span>
                  <span class="price-now">29 900 ₽</span>
                </div>
                <div class="price-icons-wrapper">
                  <a href="#" class="card-icon">
                    <svg  width="20" height="20">
                      <use href="img/sprite.svg#card-basket"></use>
                    </svg>
                  </a>
                  <a href="#" class="card-icon">
                    <svg  width="20" height="20">
                      <use href="img/sprite.svg#card-heart"></use>
                    </svg>
                  </a>
                </div>
              </div>
             <div class="button-wrapper">
              <button class="button card-button" data-toggle="modal" data-target="#feedback-modal"><span class="card-button-text">Записаться на тест-драйв</span></button>
             </div>
            </div>
        </div></div>
        <div class="swiper-slide"> <div class="card card-even  ">
          <div class="card-mark"><span class="card-mark-text">Новинка</span></div>
          
          <div class="card-top ">
            <a href="#" class="card-icon card-icon-light">
              <svg class="section-icon"  width="20" height="20">
                <use href="img/sprite.svg#scales-card"></use>
              </svg>  
            </a>
            <picture>
    <source type="image/webp" srcset="img/scooter-card.webp">
    <source  type="image/png" srcset="img/scooter-card.png">
    <img src="img/scooter-card.png" alt="scooter photo" class="card-image">

    </picture>
              
           
          </div>
  
            <div class="card-bottom">
              <h4 class=" card-title">Kugoo Kirin M4</h4>
              <ul class="card-description-wrapper">
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#accum"></use>
                  </svg>
                  <p class="card-description-text">2000 mAh</p>
                </li>
                
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#speedometer"></use>
                  </svg>
                  <p class="card-description-text">60 км/ч</p>
  
                </li>
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#card-light"></use>
                  </svg>
                  <p class="card-description-text">1,2 л.с.</p>
  
                </li>
                <li class="card-description">
                  <svg class="card-bottom-icon"  width="18" height="18">
                    <use href="img/sprite.svg#timer"></use>
                  </svg>
                  <p class="card-description-text">5 часов</p>
  
                </li>
              </ul>
              <div class="price-block-wrapper">
                <div class="price-wrapper">
                  <span class="last-price">39 900 ₽</span>
                  <span class="price-now">29 900 ₽</span>
                </div>
                <div class="price-icons-wrapper">
                  <a href="#" class="card-icon">
                    <svg   width="20" height="20">
                      <use href="img/sprite.svg#card-basket"></use>
                    </svg>
                  </a>
                  <a href="#" class="card-icon">
                    <svg   width="20" height="20">
                      <use href="img/sprite.svg#card-heart"></use>
                    </svg>
                  </a>
                </div>
              </div>
             <div class="button-wrapper">
              <button class="button card-button" data-toggle="modal" data-target="#feedback-modal"><span class="card-button-text">Записаться на тест-драйв</span></button>
             </div>
            </div>
        </div></div>
        ...
      </div>
     
      <div class="sliper-buttons-wrapper">
        <div class="sliper-button-prev">
          <svg   width="36" height="24">
            <use href="img/sprite.svg#arrow-prev"></use>
          </svg>
        </div>
      <div class="sliper-button-next">
        <svg   width="36" height="24">
          <use href="img/sprite.svg#arrow-next"></use>
        </svg>
      </div>
      </div>
      
    </div> 
 </div>
</div>

</section>


<section class=" cta">
  <picture>
    <source type="image/webp" srcset="img/woman-cta.webp">
    <source  type="image/png" srcset="img/woman-сta.png">
    <img  class="cta-image" src="img/woman-cta.png" alt="call to action">
    </picture>
  <div class=" section bg-cta">
  
<div class=" container">

      <div class="cta-content">
         <h2 class="section-title сta-title">Нет нужной модели, которую хотите протестировать?</h2>
        
        <form  id="form" action="handler.php" method="POST"  class=" send-phone cta-form ">
        

       <p class="cta-form-text">
          Оставьте заявку, и менеджер подберет нужный самокат
        </p>
          <div class="input-group-wrapper cta-input-group-wrapper">
        <div class="input-group cta-input-group ">
          <label for="user-phone" class="label"></label>
          <input name="userphone"
           id="user-phone" 
          
          type="tel" 
        class="input-cta" 
        placeholder="+7 (___) __ - __ - __"
        required
        maxlength="30">
        </div>
        <button  type="submit" class="button cta-button"><span class="button-text cta-button-text">Оставить заявку на тест-драйв</span> <span class="button-text cta-button-text-mobile">Оставить заявку </span></button>
        </div>
        <div class="notify">
      <label class="cta-label">
        <input type='checkbox'
        name="usercheck">
          <span></span>
          <p class="notify-text">Нажимая на кнопку, вы соглашаетесь на обработку персональных данных и <a href="#" class="policy-link"> политикой конфиденциальности</a></p>
          </label>
        
      </div>
       
      
      </form>
      

    </div>
    </div>
    </div>
</section>


<footer class="footer">
  <div class="footer-cta">
  <div class="container">
      <div class="footer-cta-wrapper">
        <p class="footer-cta-text-desktop">Оставьте свою почту и станьте первым, кто получит скидку на новые самокаты</p>
        <p class="footer-cta-text-mobile">Оставьте свою почту и  получите скидку </p>
   

        <form id="form"  action="handler.php" method="POST"  class=" send-mail cta-form ">
            <div class="footer-input-group-wrapper">
            <div class="input-group footer-input-group ">
              
            <input id="user-mail" 
            type="email" 
            class="input-footer" 
            name="usermail"
            required
            pattern="^([^ ]+@[^ ]+\.[a-z]{2,6}|)$"
            placeholder="Введите Ваш email"
            >
            </div>
            <button class=" footer-cta-button"><span class=" footer-button-text">Подписаться</span></button>
            </div>
           
          
          </form>
      </div>

  </div> 
  </div> 
  <div class="footer-menu">
          <div class="container">
    <div class="footer-menu-contacts-container">
      <div class="footer-menu-container">
        <div class="footer-menu-wrapper">
          <h4 class="footer-menu-title">Каталог товаров</h4>
          <ul class="footer-menu-list">
           <li class="footer-menu-item">
             <a class="footer-menu-link" href="#">Электросамокаты</a>
           </li>
           <li class="footer-menu-item">
             <a class="footer-menu-link" href="#">Электроскутеры</a>
           </li>
           <li class="footer-menu-item">
             <a class="footer-menu-link" href="#">Электровелосипеды</a>
           </li>
           <li class="footer-menu-item">
             <a class="footer-menu-link" href="#">Электровелосипеды</a>
           </li>
         </ul>
        </div>
        <div class="footer-menu-wrapper ">
          <h4 class="footer-menu-title">Покупателям</h4>
          <ul class="footer-menu-list footer-column-2">
           <li class="footer-menu-item">
             <a class="footer-menu-link" href="#">Сервисный центр</a>
           </li>
           <li class="footer-menu-item">
             <a class="footer-menu-link" href="#">Доставка и оплата</a>
           </li>
           <li class="footer-menu-item">
             <a class="footer-menu-link" href="#">Рассрочка</a>
           </li>
           <li class="footer-menu-item">
             <a class="footer-menu-link" href="#">Тест-драйв</a>
           </li>
           <li class="footer-menu-item">
             <a class="footer-menu-link" href="#">Блог</a>
           </li>
           <li class="footer-menu-item">
             <a class="footer-menu-link" href="#">Сотрудничество</a>
           </li>
           <li class="footer-menu-item">
             <a class="footer-menu-link" href="#">Контакты</a>
           </li>
           <li class="footer-menu-item">
             <a class="footer-menu-link" href="#">Акции</a>
           </li>
         </ul>
        </div>
        <!-- footer-menu-wrapper  -->
       </div>
       <!-- footer-menu-container  -->

       <div class="footer-menu-contacts-wrapper">
        <h4 class="footer-menu-title">
          Контакты
        </h4>
      <div class="contacts-wrapper">
        <div class="call-center">
          <p class="contacts-text">Call-центр</p>
          <a href="tel:+78005055461" class="footer-phone">+7 (800) 505-54-61</a>
          <span class="contacts-description">Пн-Вс 10:00 - 20:00</span>
        </div>
        <div class="service-center">
          <p class="contacts-text">Сервисный-центр</p>
          <a href="tel:+78005055461" class="footer-phone">+7 (499) 350-76-92</a>
          <span class="contacts-description">Пн-Вс 10:00 - 20:00</span>
        </div>
      </div>
       </div>
       <a href="#" class="footer-call">Заказать звонок</a>
    </div>
    <!-- footer-menu-contacts-container  -->
    </div>
    <!-- container  -->
  </div>
  <!-- footer-menu  -->
    <div class="footer-social">
      <div class="container">
        <div class="footer-social-wrapper">
            <div class="footer-app-logo-wrapper">
              <a href="./" class=" footer-logo">KUGOO</a>
              <div class="social-app-wrapper">
                  

                  <a href="#" class="google-play-link">
                    <picture>
                      <source type="image/webp" srcset="img/google-play.webp">
                      <source  type="image/png" srcset="img/google-play.png">
                      <img  class="google-play-image" src="img/google-play.png" alt="google-play">
                      </picture>
                  </a>
                  <a href="#" class="appstore-link">
                    <picture>
                      <source type="image/webp" srcset="img/appstore.webp">
                      <source  type="image/png" srcset="img/appstore.png">
                      <img  class="appstore-image" src="img/appstore.png" alt="appstore">
                      </picture>
                  </a>
               
                
               

              </div>
            </div>



          <div class="social-wrapper">
            <a href="#" class="social-block">
              <svg class="social-icon"  width="20" height="20">
                <use href="img/sprite.svg#vk"></use>
              </svg>
              <div class="social-content">
                <p class="social-media-text">Вконтакте</p>
                <span class="social-media-description">3 300</span>
              </div>
            </a>
            <a href="#" class="social-block">
              <svg class="social-icon"  width="20" height="20">
                <use href="img/sprite.svg#yb"></use>
              </svg>
              <div class="social-content">
                <p class="social-media-text">YouTube</p>
                <span class="social-media-description">3 603</span>
              </div>
            </a>
            <a href="#" class="social-block">
              <svg class="social-icon"  width="20" height="20">
                <use href="img/sprite.svg#tg-blue"></use>
              </svg>
              <div class="social-content">
                <p class="social-media-text">Telegram</p>
                <span class="social-media-description">432</span>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>

    <div class="footer-bottom">
      <div class="container">
        <div class="footer-bottom-wrapper">
          <div class="footer-policy-wrapper">
            <a href="#" class="requisite">Реквизиты</a>
            <a href="#" class="policy">Политика конфиденциальности</a>
          </div>
          <div class="payments-wrapper">
           <div class="payment-icons">
            <svg class="social-icon"  width="34" height="23">
              <use href="img/sprite.svg#pp"></use>
            </svg>
            <svg class="social-icon"  width="34" height="23">
              <use href="img/sprite.svg#appay"></use>
            </svg>
            <svg class="social-icon"  width="34" height="23">
              <use href="img/sprite.svg#visa"></use>
            </svg>
            <svg class="social-icon"  width="34" height="23">
              <use href="img/sprite.svg#mcd"></use>
            </svg>
            <svg class="social-icon"  width="34" height="23">
              <use href="img/sprite.svg#mtd"></use>
            </svg>
            
            <svg class="social-icon"  width="34" height="23">
              <use href="img/sprite.svg#wbm"></use>
            </svg>
            <svg class="social-icon"  width="34" height="23">
              <use href="img/sprite.svg#qiwi  "></use>
            </svg>
           </div>

            <span class="footer-contacts">Online чат:</span>
            <a href="#" class="footer-logo-link">
              <svg class="footer-bottom-icon"  width="16" height="16">
                <use href="img/sprite.svg#viber"></use>
              </svg>
            </a>
            <a href="#" class="footer-logo-link">
              <svg class="footer-bottom-icon"  width="16" height="16">
                <use href="img/sprite.svg#whats"></use>
              </svg>
            </a>
            <a href="#" class="footer-logo-link">
              <svg class="footer-bottom-icon"  width="16" height="16">
                <use href="img/sprite.svg#tg"></use>
              </svg>
            </a>


          </div>

        </div>
      </div>
    </div>
</footer>




  
  <div  class="modal " id="feedback-modal" >
    <div class="modal-dialog">
      <a href="#" class="modal-close" data-toggle="modal"  data-target="#feedback-modal"   >
        <svg class="close-icon"  width="20" height="20">
          <use href="img/sprite.svg#close"></use>
        </svg>
      </a>
      <div class="modal-wrapper">
        <div class="modal-content">
          <h2 class="modal-title">Запишитесь на тест-драйв электросамоката</h2>
      <p class="modal-title-description">
        и подберите модель для себя
      </p>
     
      <p class="modal-text">
        Менеджер свяжется с вами в течение 5 минут, чтобы согласовать время.
      </p>
      <form  id="form" action="handler.php" method="POST" class=" send-phone modal-form ">
        <label class=" modal-input-label " for="modal-user-phone">Как с вами удобнее связаться?</label>
        <div class="input-group modal-input-group">
          <input name="userphone" 
          id="modal-user-phone" 
          type="tel" 
          class="input-cta input-modal" 
          placeholder="+7 (___) __ - __ - __"
          required
          maxlength="30">
          </div>
          <button type="submit" class="button cta-button modal-button"><span class="button-text modal-button-text">Оставить заявку на тест-драйв</span></button>
          
      
          <div class="notify modal-notify">
          <label class="cta-label">
        <input type='checkbox'  name="usercheck">
          <span></span>
          <p class="notify-text modal-notify-text">Нажимая на кнопку, вы соглашаетесь на обработку персональных данных <br> и  <a href="#" class="policy-link-mobile">политикой конфиденциальности</a></p>
          </label  >
          
          </div>
        </form>
        
        </div>
        <!-- modal-content  -->
        <picture>
    <source type="image/webp" srcset="img/modal-woman.webp">
    <source  type="image/png" srcset="img/modal-woman.png">
    <img src="img/modal-woman.jpg" alt="modal-woman-img" class="modal-woman">

    </picture>
      </div>
      <!-- modal-wrapper  -->
      
    </div>
    <!-- modal dialog  -->
  </div>




  <div  class="modal " id="alert-modal" >
    <div class="modal-dialog">
      <a href="#" class="modal-close" data-toggle="modal"  data-target="#alert-modal">
        <svg class="close-icon"  width="20" height="20">
          <use href="img/sprite.svg#close"></use>
        </svg>
      </a>
      <div class="modal-wrapper">
        <div class="modal-content">
          
        <h2 class="modal-title">Спасибо за заявку</h2>
        <a href="#" class="modal-close" data-toggle="modal" data-target="#alert-modal">
          
        </a>
        <p class="modal-text">
          Менеджер свяжется с вами в течение 5 минут, чтобы согласовать время.
        </p>
        <button type="reset" class="button cta-button modal-button " data-toggle="modal" data-target="#alert-modal"> <span class="button-text modal-button-text">Вернуться на главную</span></button>

       
      </div> 
      <picture>
    <source type="image/webp" srcset="img/modal-woman.webp">
    <source  type="image/png" srcset="img/modal-woman.png">
    <img src="img/modal-woman.jpg" alt="modal-woman-img" class="modal-woman">

    </picture>
      
      </div>
    </div>
  </div>

  <script src="js/just-validate.production.min.js"></script>
  <script src="https://unpkg.com/imask"></script>
  <script src="js/swiper-bundle.min.js"></script>
  <script src="js/main.js"></script>
</body>
</html>